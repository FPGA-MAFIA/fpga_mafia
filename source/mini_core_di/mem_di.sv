...
module mem #(
    ...
    //interface b
    input  logic [ADRS_WIDTH-1:2] address_c  ,
    input  logic                  wren_c     ,
    input  logic [3:0]            byteena_c  ,
    input  logic [WORD_WIDTH-1:0] data_c     ,
    output logic [WORD_WIDTH-1:0] q_c   
);
...
always_comb begin
    ...
    if(wren_c) begin
        if(byteena_c[0]) next_mem[address_c_byte+0]= data_c[7:0];
        if(byteena_c[1]) next_mem[address_c_byte+1]= data_c[15:8];
        if(byteena_c[2]) next_mem[address_c_byte+2]= data_c[23:16];
        if(byteena_c[3]) next_mem[address_c_byte+3]= data_c[31:24]; 
    end
end 
...
assign pre_q_c = {mem[address_c_byte+3], mem[address_c_byte+2], mem[address_c_byte+1], mem[address_c_byte+0]};

// sample the read - synchorus read
`MAFIA_DFF(q_a, pre_q_a, clock)
`MAFIA_DFF(q_b, pre_q_b, clock)
`MAFIA_DFF(q_c, pre_q_c, clock)
