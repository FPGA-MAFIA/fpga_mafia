...
void startGame(FlappyBirdGame* game) {
    ...
    unsigned int seed = 42; // You can choose any arbitrary value here
    // Generate a random height for the first pipe
    game->pipes[0].height = myRand(&seed) % (SCREEN_HEIGHT - PIPE_GAP - 100) + 100;
    game->numPipes = 1;
...
void updateGame(FlappyBirdGame* game) {
    if (game->state == PLAYING) {
        ...
        if (game->pipes[game->numPipes - 1].x < SCREEN_WIDTH - 400) {
            ...
            game->pipes[game->numPipes].height = rand() % (SCREEN_HEIGHT - PIPE_GAP - 100) + 100;
            game->numPipes++;
        }
    }
}
...
void updateBestWeights(FlappyBirdGame* game) {
    for (int i = 0; i < game->numBirds; i++) {
        if (game->birds[i].alive) {
            ...
            game->bestWeights = game->birds[i].network.weights;
            break;
        }
    }
}
