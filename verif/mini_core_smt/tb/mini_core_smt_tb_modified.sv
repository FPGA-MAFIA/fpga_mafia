...
//FIXME - dont know why need to include the common_pkg.. its already included in the the mini_core_smt_pkg
...
initial begin: test_seq
    ...
    file = $fopen({"../../../target/mini_core_smt/tests/instz_mem.sv"}, "r");
    if (!file) begin
        $error("the file: ../../../target/mini_core_smt/tests/instz_mem.sv does not exist");
    ...
    file = $fopen({"../../../target/mini_core_smt/tests/dataz_mem.sv"}, "r");
    if (file) begin
        $fclose(file);
...
//============================================
 ...
 .mini_core_smt_ready       (),  // output  logic  mini_core_smt_ready       ,
 //
...
.clk    (Clk),
.rst    (Rst),
.run    (1'b1) // FIXME - set the RUN only when the mini_core_smt DUT is retiring the instruction.
