...
class Test:
        ...
        if self.path:
            ...
            try:
                if not self.assembly:
                    first_cmd  = 'riscv-none-elf-gcc -S -ffreestanding -march=rv32i ../../../../../'+self.path+' -o '+cs_path
                ...
                try:
                    second_cmd = 'riscv-none-elf-gcc  -O3 -march=rv32i -T ../../../../../app/link.common.ld\
                    ...
                    try:
                        third_cmd  = 'riscv-none-elf-objdump -gd {} > {}'.format(elf_path, txt_path)
                        ...
                        try:
                            forth_cmd  = 'riscv-none-elf-objcopy --srec-len 1 --output-target=verilog '+elf_path+' inst_mem.sv' 
    ...
    def _compile_hw(self):
        ...
        if not Test.hw_compilation:
            comp_sim_cmd = 'vlog -lint -f ../../../'+TB+'/'+self.project+'_list.f'
            try:
                #FIXME - see why the linux version of subprocess.check_output doesn't work
    ...
    def _start_simulation(self):
        ...
        sim_cmd = 'vsim work.'+self.project+'_tb -c -do "run -all" +STRING='+self.name
        try:
    ...
    def _gui(self):
        os.chdir(MODELSIM)
        gui_cmd = 'vsim -gui work.'+self.project+'_tb +STRING='+self.name+' &'
        try:
